...
<div class="modal-backdrop" (click)="dismissed.emit()">
    ...
    @if (selectedInvoice()) {
      <div class="amount-section">
        ...
        <div class="amount-hint">
          Total transaction : {{ tx().montant | displayCurrency : tx().devise }}
          @if (partialAmountValue > 0 && partialAmountValue < tx().montant) {
            &nbsp;— Reste : <strong>{{ (tx().montant - partialAmountValue) | displayCurrency : tx().devise }}</strong>
          }
        </div>
        @if (amountError()) {
          <div class="amount-error">{{ amountError() }}</div>
        }
      </div>
    }
  ...
  styles: [`
    ...
    .amount-section { display: flex; flex-direction: column; gap: 0.375rem; }
    .amount-label { font-size: 0.8rem; font-weight: 500; color: #374151; }
    .amount-row { display: flex; align-items: center; gap: 0.5rem; }
    .amount-input {
      height: 36px; padding: 0 0.75rem; border: 1px solid #cbd5e1; border-radius: 6px;
      font-size: 0.875rem; color: #0f172a; width: 160px; text-align: right;
      &:focus { outline: none; border-color: #3b82f6; box-shadow: 0 0 0 2px rgba(59,130,246,0.15); }
    }
    .amount-devise { font-size: 0.8rem; font-weight: 600; color: #64748b; }
    .amount-hint { font-size: 0.8rem; color: #64748b; }
    .amount-error {
      font-size: 0.8rem; color: #991b1b; background: #fef2f2;
      border: 1px solid #fecaca; border-radius: 4px; padding: 0.375rem 0.625rem;
    }
    .modal-error {
    ...
    .btn-confirm {
      ...
      background: #0369a1; color: #fff; font-size: 0.875rem; font-weight: 600; cursor: pointer;
      &:hover:not(:disabled) { background: #075985; }
      &:disabled { opacity: 0.5; cursor: default; }
    }
  `],
})
export class PartialMatchModalComponent {
  ...
  selectInvoice(inv: InvoiceListItem): void {
    ...
    this.partialAmountValue = this.tx().montant;
  ...
  confirm(): void {
    ...
    this.svc.confirmPartialMatch(this.tx().id, this.selectedInvoice()!.id, this.partialAmountValue).subscribe({
