...
<div class="space-y-5">
  ...
  @if (affaire.contractAmount) {
    <div class="inline-flex items-center gap-2 text-xs text-[#44474c] bg-[#f0f4f8] rounded-lg px-3 py-1.5">
      ...
      Montant du contrat :
      <strong>{{ affaire.contractAmount | displayCurrency : affaire.devise }}</strong>
        ...
        <tbody class="divide-y divide-[#f1f5f9]">
          @for (t of history(); track t.id) {
            <tr class="hover:bg-[#f8fafc] transition-colors">
              ...
              <td class="px-4 py-3 text-right text-[#1d2b3e]">{{ t.valeurCalculee | displayCurrency : affaire.devise }}</td>
              <td class="px-4 py-3">
        ...
        <div>
          ...
          @if (affaire.contractAmount && newTaux > lastValidated()) {
            <p class="text-xs text-[#1a6b7c] mt-1">
              Valeur incrémentale :
              <strong>{{ (newTaux - lastValidated()) / 100 * affaire.contractAmount | displayCurrency : affaire.devise }}</strong>
            </p>
          }
        </div>
        <div>
