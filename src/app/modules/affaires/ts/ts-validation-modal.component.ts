...
@Component({
  ...
  imports: [FormsModule, BodyPortalDirective, DisplayCurrencyPipe],
  template: `
        ...
        <div class="modal-body">
          @if (config().step === 'commerciale') {
            <div class="confirm-banner">
              Cette action intégrera <strong>{{ config().montant | displayCurrency : config().devise }}</strong>
          ...
          <div class="ts-info">
            ...
            <span class="ts-amount">{{ config().montant | displayCurrency : config().devise }}</span>
          </div>
